--exports['qb-core']:GetCoreObject() --deactivate this if you use ESX
ESX = exports["es_extended"]:getSharedObject() --deactivate this if you use qbcore
...
function ToggleClothing(whic, extra)
	...
	if which ~= "Mask" then
		if not Gender then Notify(Config.Locales['wrong_ped']) return false end -- We cancel the command here if the person is not using a multiplayer model.
	end
	...
	if not Toggle.Table.Standalone then -- "Standalone" is for things that dont require a variant, like the shoes just need to be switched to a specific drawable. Looking back at this i should have planned ahead, but it all works so, meh!
		...
		Notify(Config.Locales['started_mission']) return
	else
		if not LastEquipped[which] then
	...
	Notify(Config.Locales['already_wearing']) return false
end
...
function ToggleProps(whic)
	...
	if not Prop.Variants then
		...
		Notify(Config.Locales['nothing_to_remove']) return false
	else
		local Gender = IsMpPed(Ped)
		if not Gender then Notify(Config.Locales['wrong_ped']) return false end -- We dont really allow for variants on ped models, Its possible, but im pretty sure 95% of ped models dont really have variants.
		...
		Notify(Config.Locales['no_variants']) return false
	end
end
...
--If you want to edit the radial menu you can do that here--
lib.registerRadial({
	id = 'extrasmenu',
	items = {
	 {
